...
using Func = FunctionData<double>;
...
void CalculateInterpolation(Func &&f, int start, int end)
{
    ...
    auto range = Utils::Range<double>::Fixed(0, 30, 0.25);

    Func newton_poly = Interpolation::NewtonPolynomial(f, range);

    Func direct_poly = Interpolation::DirectPolynomial(f, range);
