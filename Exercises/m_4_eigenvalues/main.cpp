...
int main() {
  ...
  LOG_ASSERT(false, "prova", utils::WARN);
  ...
  ComplexSolutionPair solution_pair_inv = eigen::InversePowerMethod(A, 100);
  ...
  auto Ax2 = (1.0 / solution_pair_inv.first) * A.Dot(solution_pair_inv.second);
  tensor::Print(Ax2);
...
ComplexSolutionPair new_solution_pair_inv = eigen::InversePowerMethod(A, n);

std::cout
    << n << " "
    << std::abs(new_solution_pair.first / solution_pair.first - 1.0) << " "
    << std::abs(new_solution_pair_inv.first / solution_pair_inv.first - 1.0)
    << std::endl;
}
  */

  /*
auto [eigenvalues, eigenvectors] = eigen::Deflation(A, 10000);

for (int i = 0; i < eigenvalues.size(); i++) {
  std::cout << "Eigenvalue: " << eigenvalues[i] << std::endl;
  tensor::Print(eigenvectors[i]);
}
